(ns restaurantops.operation
  ...
  expressed as a langgraph-clj StateGraph. The advisor (RestaurantAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the RestaurantGovernor (:govern) and the rollout phase gate
  ...
  operator/restaurant coordinator. The approver resumes with `{:approval
...
(defn build
  ...
  `restaurantops.store/Store`).
  opts:
    :advisor      -- a `restaurantops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RestaurantAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; RestaurantGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :location-id (:location-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :location-id (:location-id request) :by (:by approval)}]}
            {:disposition :hold
